...
def train(model, train_dataloader, val_dataloader, device, config):
    # -------------------------------------------------------------------------------------------------------------------
    # Declare loss and move to device (both smoothl1 and pure l1 losses)
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            reconstruction = model(batch['input.sdf'])
...
def main(config):
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        # worker_init_fn=train_dataset.worker_init_fn # -- Uncomment this line if you are using shapenet_zip on Google Colab
    )
    ...
    val_dataloader = torch.utils.data.DataLoader(
        ...
        num_workers=4,   # Data is usually loaded in parallel by num_workers
        pin_memory=True,  # This is an implementation detail to speed up data uploading to the GPU
        # worker_init_fn=val_dataset.worker_init_fn # -- Uncomment this line if you are using shapenet_zip on Google Colab
    )
