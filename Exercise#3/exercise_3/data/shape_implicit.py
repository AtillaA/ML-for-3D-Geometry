...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        # Sample positive and negative sdf values / points
        random_pos = (torch.rand(half) * pos_tensor.shape[0]).long()
        random_neg = (torch.rand(half) * neg_tensor.shape[0]).long()

        # Tensor with approximately self.num_sample_points/2 randomly selected samples from pos_tensor
        sample_pos = torch.index_select(pos_tensor, 0, random_pos)

        # Tensor with approximately self.num_sample_points/2 randomly selected samples from neg_tensor
        sample_neg = torch.index_select(neg_tensor, 0, random_neg)

        samples = torch.cat([sample_pos, sample_neg], 0)

        return samples # Pytorch float32 torch tensor of shape (self.num_sample_points, 4)
        # ---------------------------------------------------------------------------------------------
